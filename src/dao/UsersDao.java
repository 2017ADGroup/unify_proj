...
public class UsersDao {
	...
	private static final String SQL_SELECT_BY_PROPERTY = "SELECT * FROM users WHERE property=? AND organization=?";
		...
		try (PreparedStatement stmt = connection.prepareStatement(SQL_SELECT_ID_AND_PASS)) {
			...
			if (rs.next()) {
				return new Users(rs.getInt("user_id"), rs.getString("login_id"), rs.getString("password"),
						rs.getInt("property"), rs.getString("name"), rs.getString("kana"), rs.getInt("authority"),
